...
- [X] Linearer Fit Standard
- [X] Aufbau Neuronales Netz
- [X] Resultat
- [X] Overfitting
- [X] Data Augmentation
- [X] Results
- [X] Shift to higher Values
- [X] Kink Border Problem
...
Es besitzt 659,465 freie Parameter und jede Lage besteht aus Nodes die
mit allen Nodes der vorherigen Lage verbunden sind. Bis auf die Output
Node haben alle Nodes ReLU als Aktivierungsfunktion.

Das zweite Netwerk besteht aus einem Convolutional Teil an dem ein
fully connected Netwerk angeschlossen ist. Die Lagenstruktur ist in
Tabelle ref:conv_structure dargestellt.
...
Die alternative Struktur reduziert die Anzahl der freien Parameter auf 78,414.
...
Um die unphysikalischen Abweichungen am Ende der Verteilung zu
reduzieren, sollte ein adversarial network eingesetzt werden. Die
theoretischen Grundlagen für diese Art von Training sind in dem
Article "Learning to Pivot with Adversarial Networks" by G. Louppe et
al. zu erst beschrieben worden. Hier wurde eine Modifikation Ihres
Trainings Prozesses angewandt.

Die grundlegende Idee hier war, dass aus dem Resultat des neuronalen
Netzes eine Größe errechnet werden soll, die für alle Werte unabhängig
sein soll. In diesem Fall ist das angestrebte Ziel einen konstante
Verteilung der Werte von
$(E_{\text{pred}}-E_{\text{true}})/\sqrt{E_{\text{true}}}$. Hierfür
wurde an das vortrainierte Neuronale Netz ein feindliches Netzwerk
angehängt, dessen Ziel ist ist aus dieser Größe auf $E_{\text{pred}}$
zurückzuschließen. Der Trainings-Aufbau ist in Abbildung
\cite{adv-training} dargestellt. Das feindliche Netzwerk wurde mit einer Categorial Cross Entropie als 
Loss Funktion 
...
trainiert. Hier sind $f$ und $r$ die Funktion des Netzwerkes und
seines Feindes. $\mathcal{C}$ ist die Funktion, die alle Werte
von 0. bis 10. GeV in 20 gleich große Kategorien unterteilt und dann
jeder Kategorie auf einen 20-dimensionalen Basisvektor abbildet.

Es wird nun abwechselnd das Neuronale Netz und sein Adversary trainiert.
Das Neuronale Netz wird mit der Loss Funktion

\begin{align}
\mathcal{L}_f = \sum (Y_{\text{true}} - f(X))^2 - \lambda \mathcal{L}_r
\end{align}

trainiert. Durch das negative Einbinden des Adversary versucht das
Netzwerk die Performance des gegnerischen Netzes möglichst schlecht zu
machen.

Beim Trainieren des Adversary werden die Gewichte des
ersten Netzes festgehalten und umgekehrt. $\lambda$ ist ein beliebig
festzulegender Faktor, der hier auf 2.5 festgelegt wurde.

Der Trainingverlauf bestand aus einem abwechselnden Trainieren beider
Netzwerke für jeweils 5 Epochs. Dieser Ablauf wurde zehn mal
wiederholt und anschließend die Ergebnisse für das test sample des
neuronalen Netzes aufgenommen. 
...
Dieser beschriebene Vorgang wurde neun Mal wiederholt und damit neun
Resultate erzeugt, die in Abbildung ref:adv_scatter dargestellt sind.

Zu sehen ist, dass die Ergebnisse fluktuieren und sich nicht
konvergieren. Bis auf zwischenzeitlich auftretende Anomalien verändert
sich das Endresultat kaum merklich.

** TODO Likelihood Solution

Da das Adversarial Training nicht zu gewünschten Ergebnissen führte
wurde sich in diesem Teil der Kompensation der Abweichungen in der
Distribution mittels des Verändern der Loss Funktion angeschaut.

Auch im vorherigen Teil kam eine modifizierte Loss Funktion zum
Einsatz.  Wie aus der unterliegenden Verteilung der Werte eine
adequate Loss Funktion ermittelt werden kann, soll hier skizziert
werden.

Durch Umformen und zulässige Hinzufügungen lässt sich aus dem mean
squared error eine maximum likelihood herleiten
...
Der Annahme das der mean squared error die gültige
Mimimierungsfunktion des Neuronalen Netzes ist liegt also die Annahme
zu Grunde, dass die Werte Gauß verteilt sind und einen konstante
Standardabweichung besitzen.

Für ein Kalorimeter ist aber keine konstante Standardabweichung
gegeben. Außerdem ist auch die Annahme dass die Werte eine volle
Gauß-Verteilung besitzen an den Rändern nicht gültig. Hier muss die
Abwesenheit der Datenpunkte mit einer Normierung kompensiert werden.
Diese Auswirkungen sind in Abbildung ref:gaussian-shift dargestellt.

#+CAPTION: Die unterschiedlichen Likelihoods zweier Gauß Funktionen sind dargestellt. Einmal mit Normierung und einmal ohne Normierung.
#+NAME: gaussian-shift
[[../images/gaussian_shift.png]]

Um also die abgeschnittene Verteilung zu berücksichtigen nehmen wir als Wahrscheinlichkeitsdichtefunktion 
\begin{align}
...
wobei $a$ und $b$ hier 0. und 10 GeV sind. Wir können hieraus dann
eine Loss Funktion herleiten, die es für das Neuronale Netz zu
minimieren gilt.
...
&=\text{min}  \sum \frac{(x-\mu)^2}{\sigma^2} + \ln(\frac{\pi \sigma^2}{2}\left(\text{erf}\left(\frac{\mu-a}{\sqrt{2}\sigma}\right) - \text{erf}\left(\frac{\mu-b}{\sqrt{2}\sigma}\right)\right)^2) 
\end{align}

Das resultierende Loss Funktion besteht aus einem Teil des mean
weighted squared errors, diese Gewichtung verhindert die globale
Verschiebung der Werte nach oben. Da Werte mit hohen Energien die
selben Beiträge zur gesamt loss funktion beitragen wie Werte mit
geringen Energien. Der zweite Term ist ein Korrekturfaktor der die
Enden der Verteilung berücksichtigt und deren Effekte reduziert.
