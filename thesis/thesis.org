...
Der Inhalt dieses Kapitels ist die Jet Energy Calibrierung mittels
neuronalen Netzen dargestellt.  Die Anwendung der im vorhergegangen
Methodik zur Reduzierung von Randeffekten auf Jets wird vorgestellt.
Neben einer einfachen Netzwerkarchitektur wird auch ein Particle Flow
Netzwerk zur Regression verwendet.

In dieser Analyse wurden ur Daten verwendet die durch Simulation
entstanden sind. Das betrachtete Datensample ist ein QCD Datensatz der
zur 2018 Detektorsimulation gehört. Der Datensatz besaß ein $P_T$
Spektrum von 15 bis 3000 Gev. Der Datensatz lag ohne PileUp vor.

Zur Analyse wurden aus dem Datensatz einzelne Jets extrahiert. Es
wurden nur Jets im Energiebereich zwischen 30 und 140 GeV ausgewählt.
Die Verteilung der 

\todo{wieviele Jets, split?}


Zu jedem Event wurden die Vierer Vektoren der konstituierenden
Teilchen gespeichert. Es wurde die transversalen Impulse der Jets auf
Generatorlevel und auf nach der Simulation aufgezeichnet.

Das Ziel dieses Teiles ist es nun von der dem Satz an Teilchen auf den
Impuls zurück zu schließen. 

** Deep Learning Setupn
...
- R einführen
Als Maß für die Calibrierung wurde die Variable $R$ verwendet,

\begin{align}
R = \frac{P_{T, \text{Reco}}}{P_{T, \text{Gen}}}.
\end{align}

$P_{T, \text{Gen}}$ ist der transversale Impuls der Jets auf
Generatorlevel und P_{T, \text{Reco}} ist der rekonstruierte
tranversale Impuls des Jet Algorithmuses oder des Neuronalen Netzes.

#+CAPTION: Die resultierenden $R$ Punkte des Reco Algoritmus und des Neuronalen Netzes im Verhältnis zu $P_{T, \text{Gen}}$ sind dargestellt. 
#+NAME: jetnet_R_scatter
[[../images/jetnet_R_scatter.pdf]]

In der Abbildung ref:jetnet_R_scatter sieht man die Ergebnisse des
Reco Algorithmuses und des Neuronalen Netzes in einem Scatter Plot
übereinander dargestellt. Als Loss Funktion für das Neuronale Netz
wurde MSE verwendet. Durch das beschnittene Wertespektrum sieht man
die ähnliche systematische Abweichungen wie schon im vorhergegangen
Kapitel.  

Um diese zu beheben nutzen wir nun die Kraft der Imagination und des
Verstandes.  Ne Spaß, wir machen natürlich das gleiche, was wir auch
im letzten Kapitel gemacht haben. Wäre ja Quatsch sich nochmal
Gedanken zu machen.

#+CAPTION: Die Mittelwerte der R Verteilung sind in Abhängigkeit zu $P_{T, \text{Gen}}$ dargestellt. Reco steht für den Reco Algorithmus. MSE für das Neuronale Netz mit 'mse' als Loss Funktion. 1st und 2nd stehen für die erste und zweite Iteration mit abweichender Loss Funktion.
#+NAME: jetnet_R
[[../images/jetnet_R.pdf]]


Wie oben beschrieben, ich hoffe ich habe das auch oben beschrieben,
werden in Abbildung ref:jetnet_R die resultierenden R Werte für die
neuronalen Netze und den Reco Algorithmus dargestellt. Man sieht, dass
die erste Interation die systemamtischen Abweichungen an den Rändern
behebt, allerdings zu hohe Ergebnisse liefert. Dieser Shift zu hohen
Werten findet sich nicht nach der zweiten Iteration. Die Randeffekte
konnten nicht behoben werden und bestehen auch nach mehrfacher
Iteration. 
...
In Abbildung ref:jetnet_res ist die Auflösung dargestellt. Man sieht
das alle Netze besser performen als der Reco Algorithmus. An den
Rändern sinkt wieder die Auflösung. Dies kann durch die alternative
Loss Funktion eingedämmt werden.

** Particleflownet

- Performance Plot
- R Plot Systematik
- R für verschiedene Variablen


Im nächsten Teil wurde ein PFNet trainert. Der Trainingsablauf war der
gleiche wie für das erste Netz. Der Aufbau des PFNets besteht aus
einem Filter Module das aus drei Lagen besteht mit (100, 100, 128) .
Das heißt wir landen in einem 128dim Output Space, der wir dann an das
zweite Netzwerk weiter gereicht. Dieser besteht aus drei Lagen mit
jeweils 100 Neuronalen gefolgt von einer Node aus Output.

#+CAPTION: Die Mittelwerte der R Verteilung sind in Abhängigkeit zu $P_{T, \text{Gen}}$ dargestellt. Reco steht für den Reco Algorithmus. MSE für das Neuronale Netz mit 'mse' als Loss Funktion. 1st und 2nd stehen für die erste und zweite Iteration mit abweichender Loss Funktion.
#+NAME: pfnet_R
[[../images/pfnet_R.pdf]]

Auf Abbildung ref:pfnet_R sieht man die Mittelwerte der R
Verteilung. Pretty dope. Das Prozedere von eben wurde auch hier auf
die Sachen losgelassen. Man sieht, dass die alternative Loss Funktion
stark dabei hilft den Teil am Anfang zu beseitigen.  Insgesamt besitzt
die gesamte Verteilung noch eine gewisse Schieflage, aber sonst ist
das eigentlich schon ganz nett.

#+CAPTION: Die Standardabweichung durch die Wurzel von $P_{T, \text{Gen}}$ in Verhältnis zu $P_{T, \text{Gen}}$ ist dargestellt.
#+NAME: pfnet_res
[[../images/pfnet_res.pdf]]

In ref:pfnet_res ist die Verteilung der der Auflösungen
dargestellt. Alle PFNet Resultate haben in etwa die gleichen Verläufe
und sind deutlich besser als das REco Teil. Die beste Performance wird
nach der zweiten Iteration erzielt
.
** Comparison

In Abb. ref:comp_R ist zu sehen, dass der Verlauf der R Verteilung der beiden Netzwerktypen fast identisch ist.
x

#+CAPTION: Die Mittelwerte der R Verteilung sind in Abhängigkeit zu $P_{T, \text{Gen}}$ dargestellt. Reco steht für den Reco Algorithmus. MSE für das Neuronale Netz mit 'mse' als Loss Funktion. Die Verläufe beider Netzwerktypen und des REco sind dargestellt.
#+NAME: comp_R
[[../images/comp_R.pdf]]
...
- input variablen nicht gleichmäßig
