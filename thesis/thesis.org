...
** DONE Energy Regression By Linear Fitting
...
Die Datenstruktur visualisiert in Abbildung ref:data_display besitzt
eine drei-dimensionale Pixelstruktur aus. Sie kann als Stapel von
Bildern interpretiert werden. Für die Auswertung von Bildern sind
Convolutional Neural Nets besonders geeignet. Deshalb wird im
folgenden eine Architektur von mehreren aufeinander folgenden
Convolutional Filtern, gefolgt von Lagen aus fully connected layern
verwendet. Auf Pooling Filter wurde verzichtet, da diese Filter die
Weitergabe quantitativer Größen im Netz verhindern. Die Lagenstruktur
ist in table xx zu sehen.

\todo{Lagenstruktur einfügen}

Falls nicht anders expliziert wurde dieses Netzwerk für die
weitergehende Analyse verwendet.
...
our predicted energy values. We train the network for 250 epochs with a
batch size of 128.
...
label:first_loss
 ...
 In figure ref:first_loss the loss function is shown. While the loss
...
Um das overfitting problem zu beheben, wurde die Technik der Data
Augmentation angewandt.  Data Augmentation in dieser Anwendung meint
das wir alle gültigen Symmetrietransformationen ausnutzen um die
Anzahl der Datepunkte zu erhöhen. Jedes Datenpunkt wird zufällig um
bis zu 3 mal um 90 Grad gedreht und zufällig die gesamten Datenpunkte
gespiegelt. Dies verachtfacht die möglichen Datenpunkte und soll das
Netzwerk dazu befähigen die Symmetrien im System zu lernen.
Anschließend wurden die Datenpunkte noch in allen Richtungen
verschoben um die Daten unabhängig ihres Eintreffpunktes zu machen.
An dieser Stelle sei bemerkt das dieses Transformationen nur gültig
sind, da unsere Simulation absolut homogen und symmetrisch ist. Bei
realen Detektorpunkten ist dies nicht gegeben.
 ...
 #+CAPTION: The Graph shows the evolution of the loss function for the training set and the validation set.
 #+NAME: Loss
label:data_augment_loss
 [[../images/data_augment_loss.pdf]]

Wie in Abbildung ref:data_augment_loss zu sehen, kann das Overfitting
für unseren Datensatz mit dieser Methode komplett reduziert werden.
Die Fluktuationen in den Werten der Loss Funktion für das validation
sample entstehen durch die höhere statistische Fluktuation einer
kleineren Datenmenge. Der globale Verlauf folgt asymptotisch dem der
loss für das trainingsset.
...
Der Scatterplot der Resultate des neuronalen Netzes und des linearen
Fits sind in ref:data_augment dargestellt. In schwarz im Vordergrund
ist das ergebnis des neuronalen Netzes abgebildet, dahinter in blau
das Resultat des linearen Fits zum Vergleich. Bis zu einem Wert von
etwa 8 GeV sind die Punkte des neuronalen Netzes und des linearen Fits
in etwa gleich verteilt. Anschließend flacht die Verteilung des
neuronalen Netzes stark ab. Dies liegt daran, dass das neuronale Netz
lernt das die Verteilung der resultierenden Datenpunkte bei einer
Grenze von 10 GeV aufhört. Es kann also Datenpunkte deren statistische
Fluktuation auch derer höherer Energien entsprechen könnte, leichter
separieren und zu niedrigeren Energien korrigieren. Diese Reduktion
der Verteilungsbreite führt zu einer systematischen Verzerrung.
 ...
 #+NAME: data_augment_res
 [[../images/data_augment_res.pdf]]

In Abbildung ref:data_augment_res wurden die gesamten Datenpunkte in
25 Bins mit gleichem Abstand ihrer resultierenden Energie sortiert
eingeteilt und für jeden Bin jeweils der Mittelwert der
Energiedifferenzen und die Standardabweichung der Energiedifferenzen
bestimmt. Im oberen Graphen ist der Mittelwert der Energiedifferenzen
aufgetragen. Im unteren Graphen ist die Standardabweichung durch die
Wurzel der resultierenden Energien abgebildet. Die Schlüsse aus dem
Scatterplot sind auch hier zu beobachten. In den Mittelwerten sieht
man den Abfall zu hohen Energien, die in einer erhöhten Auflösung
resultiert. Des weiteren ist hier zu sehen, dass die Datenpunkte
global systematisch nach oben verschoben sind.

Diese Verschiebung ist mit der Wahl des Mean Squared Errors als Loss
Funktion zu erläutern. Die einzelnen Residuen der Summationsbeiträge
sind auf Grund der statistischen Verteilung für größere Energien
größer. Das Netzwerk optimiert sich deshalb zu höheren Werten und
erzeugt systematische Fehler. 

Hier zeigt sich das die Wahl der Loss Funktion entscheident ist um
systematische Fehler vorzubeugen. Die Loss Funktion muss die Statistik
der Datenpunkte widerspiegeln. Dies wird weiterführend im Kapitel xx
diskutiert. 

** TODO Adversarial Solution                                       :noexport:

*** Gute Fragen [/]
- [ ] was habe ich eigentlich gemacht?
- [ ] wie stellen wir das da?
...
** TODO Linear Fit with different version
...
** TODO Are we learning the shape?                                 :noexport:
