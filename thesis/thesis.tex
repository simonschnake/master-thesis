...
\added{Figure \ref{fig:e-distr} shows the distribution of the energies
  of the incoming simulated particles. The energies were chosen
...
The training data set consists of 253064 simulated events and the
...
applied % whose structure is not adapted to the structure of the data.
\added{The data tensor is flatten into a 1088 dimensional vector to
  fit the input space of the FCN}.
...
\ref{fcn_structure}.
\added{Different structures were tried out. The presented version is
  the one with the best convergence.} It has 659,465 free parameters
and each layer \added{is fully connected to the previous layer}. With
the exception of the output node, all nodes have ReLU as their
activation function.
...
In this section a three-dimensional convolutional neural net is
...
\added{The convolutionaln network has 78,414 free parameters, which
  are only 12 percent of the parameters of the fully connected network
  previously used.}

\todo{überarbeiten + node / filter}
\todo{aufsplitten der beschreibung CONVNET}
\todo{idea: local pattern by conv-net followed by dense net}
\todo{wie zu struktur gekommen}
\todo{resultate einfügen}

\section{Adversarial Training}
\label{sec:org30a0273}

In order to reduce the unphysical deviations at the end of the
distribution, an adversarial network was used.

\todo{Describe modification of this training process.}

- erstes Netzwerk ConvNet
- dann größe errechnen
- zweites Netzwerk 
...
$\sqrt{9.5}$ are shown. To this data two maximum likelihood fits are
applied. One for a gaussian distribution and one for a gaussian
distribution divided  by the cumulitive gaussian distribution

\begin{align}
\frac{\frac{1}{\sqrt{2\pi \sigma^2}}
  e^{-\frac{(x-\mu)^2}{2 \sigma^2}}}{\int^{10}_{-\infty} \frac{1}{\sqrt{2\pi
...
where \(a\) and \(b\) in our case are 0.0 and 10.0 GeV. A loss function
which has to be minimized for the neural net can than be derived as
...
with high energies contribute to the total loss function as values
with low energies. The second term is a correction factor that takes
into account the ends of the distribution and reduces its effects.
