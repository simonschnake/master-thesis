...
the data set were created without pile up. \added{For comparison the $P_T$
results of the kT-Algorithm was used and denoted as $P_{T, \text{Reco}}$.}
...
\added{The regression process in this chapter is iterative. First the network is trained with MSE as loss function then the trained network is used to calculate the standard deviation $\sigma$, as it is needed for the loss function described below. With the later described loss function the network is trained again. And again $\sigma$ is determined. Then a further time $\sigma$ is determined and the network is trained. This training scheme is shown in figure \ref{fig:jet-trainings-setup}.}
...
\added{The results will be labeled as MSE, 1st and 2nd, which means the results obtained after training with the MSE Loss function, and the two subsequent training sessions with the loss function described below.}

To deduce the momentum, the 4 vectors of the particles of the jets are
used as an input for the networks. First, the dimensions of the
\((200, 4)\) input space are transformed into a (800, 1) space. In the
training process, the data was processed in batch sizes of 1024.

\added{The possible network architectures are kept here in general, they will be described later for the two analyzed cases, the JetNet and the Particle Flow Network. The basic requirement for the network is that the input layer consists of 800 inputs and that the output layer is a single linear output node.}

Since the standard deviation \(\sigma\) was not a priori apparent, it
was approximated before. For this purpose the results of the network
were calculated and divided into 20 bins according to \(P_{T,
  \text{Gen}}\). The standard deviation was then determined for each
...
algorithm. This $sigma$ function is than inserted in the loss function.

\begin{figure}[H]
  \centering
  \includegraphics[width=.9\linewidth]{../images/sigma_fit.pdf}
  \caption{\todo{plot fit}}
  \label{sigma_fit}
\end{figure}
...
\begin{align}
  \mathcal{L}_{J}(\hat Y, Y, \sigma(Y, c_{0,1,2})) = \frac{1}{n}
  \sum_{i} \frac{1}{n_{b_i} } \sum_{b_i < Y \leq b_{i+1}} \frac{(\hat
    Y -Y)^2}{\sigma^2} \nonumber \\ +
  \ln(\frac{\pi\sigma^2}{2}\left(\text{erf}\left(\frac{Y-a}{\sqrt{2}\sigma}\right)
  - \text{erf}\left(\frac{Y-b}{\sqrt{2}\sigma}\right)\right)^2).
\end{align}
...
per bin. \added{The values are sorted by \(P_{T, \text{Gen}}\) and
  ...
  the mean value was determined and the total loss is the mean of the
...
\added{where is $X$ stands for the calibration algorithm.}
...
\added{The network was trained with the training procedure described
...
\begin{figure}[H]
  ...
  \caption{The mean values of the R distribution versus \(P_{T, \text{Gen}}\).}
  \label{pfnet_R}
\end{figure}
