...
\todo{genereller Prozess}
-genereller Text
- loss function


\begin{align}
  \mathcal{L}_{J}(\hat Y, Y, \sigma(Y, c_{0,1,2}))
  = \frac{1}{n} \sum_{i} \frac{1}{n_{b_i} }  \sum_{b_i < Y \leq b_{i+1}} \frac{(\hat Y -Y)^2}{\sigma^2} + \ln(\frac{\pi
    \sigma^2}{2}\left(\text{erf}\left(\frac{Y-a}{\sqrt{2}\sigma}\right)
...
- model generell input 200x4 -> 800 output linear 1

To deduce the momentum, the 4 vectors of the particles of the jets
are used as an input for the networks. First, the dimensions of the
\((200, 4)\) input space are transformed into a (800, 1) space.

Die möglichen Netzwerk Architekturen bleiben hier generell gehalten,
sie werden später für die beiden analysierten Fälle, das JetNet und
das Particle Flow Netzwerk beschrieben. Die generelle Anforderung an
das Netzwerk ist nur, dass das Input Layer aus 800 Einträgen besteht
und dass das Output Layer ein Node ist mit einer linearen Output Node.
...
- mse
As a first step the network was trained with the mean squared
...
\todo{loss function, overtraining}
...
\begin{figure}[H]
  ...
  \caption{The mean values of the R distribution are represented as a
    ...
    function described above. \todo{überarbeiten}}
  \label{jetnet_R}
\end{figure}
