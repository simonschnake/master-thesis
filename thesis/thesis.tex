...
PbWO4 crystals and has a solid angle granularity of \(0.0174
\abs{\eta} \times 0.0174 \abs{\phi}\), providing a homogeneous
resolution. Furthermore, the ECAL covers a solid angle range of up to
\(\abs{\eta} = 3\). The lack of instrumentation from \(1.479 <
\abs{\eta} < 1.653\) is pointed out, subsequently this region is
unsuitable for the reconstruction of electrons and photons.  If the
trajectory of an electron or photon is directed through ECAL, such a
particle emits energy in the form of emitted photons and electrons
from bremsstrahlung and pair production. The emitted photons are
measured by photodiodes with a relative energy resolution \(\left(
\frac{\sigma}{E} \right)^{2}\), where sigma is the resolution of the
measured energy

\begin{align}
  \left( \frac{\sigma}{E} \right)^{2} = \left( \frac{2.8}{\sqrt{E}}
  \right)^{2} + \left( \frac{0.12}{E} \right)^{2}+(0.3)^{2}.
\end{align}
...
\(0.087[\eta] \times 0.087 [\phi]\), whereas the HF with \(0.0175
   [\eta] \times 0.0175 [\phi]\) has a much better angular resolution.
...
effects, such as interactions in the material, are reduced. The jet
corrections in CMS follow a fixed procedure.
...
is determined on the jet energy scale by events with a jet and a
photon or a Z boson. The measurement of the transverse moments of the
Z-bosons pTz and the photons \(p_T^{\gamma}\) are performed in the
well understood detector range \(\eta < \abs{1.3}\) and have much
lower uncertainties compared to the transverse moments of the jets
\(p_T^{\text{jet}}\). Thus the momentum of the jet can be balanced
with the momentum of the photon or the Z-boson. In this case the
response is according to
...
labels. This objective function is refered to as the \emph{loss
...
seen as a landscape in a hyperdimensional space spanned by the
...
hyperparameter, which value is not a priori determinable. The
parameters \(p\) are updated until a minimization criterium is
...
It is than straighforward to compute the derivation between the loss
...
basic idea behind this algorith is that for data types like pictures
...
\begin{figure}[hbtp]
  ...
  \begin{tikzpicture}[>=latex']
    \tikzset{block/.style= {rectangle, draw, fill=blue!20, text
        width=6em, text centered, rounded corners, minimum height=4em,
        node distance=3.5cm}, input/.style={ % requires library
        shapes.geometric draw, minimum width=2cm, align=center,
        minimum height=1cm }, }
...
\added{Instead of hadrons, electrons are simulated here since their
  linear energy response allows a better comparison of the network
  with a linear fit. The reconstruction of pions is the task of
  further studies.}
...
0 and 10 GeV following a flat distribution, \added{which is shown in
...
\begin{figure}[hbtp]%
  \centering
  \subfloat[complete spectrum]{{\includegraphics[width=0.45 \textwidth]{../images/sumn_distribution.pdf} }}%
  \qquad
  \subfloat[spectrum at 2 and 8 GeV]{{\includegraphics[width=0.45 \textwidth]{../images/sumn_e28_distribution.pdf} }}%
...
\begin{figure}[hbtp]%
  \centering
  \subfloat[complete spectrum]{{\includegraphics[width=0.4 \textwidth]{../images/x_distribution.pdf} }}%
  \qquad
  \subfloat[spectrum at 2 and 8 GeV]{{\includegraphics[width=0.4 \textwidth]{../images/x_e28_distribution.pdf} }}%
  \caption{\added{Distribution of summed up traces in the direction of the incoming particle}}%
  \label{fig:x-distri}%
\end{figure}

\added{Figure \ref{fig:x-distri} shows the distribution of the summed
  traces in the individual layers in the beam
  direction. \ref{fig:x-distri}a shows that electrons in the selected
  energy range with the selected calorimeter architecture almost
  exclusively interact with the first layers. In \ref{fig:x-distri}b
  you can see that the variation between 2 and 8 GeV electrons is
  visible in beam direction In \ref{fig:x-distri}b you can see that
  the variation between 2 and 8 GeV electrons is clearly visible in
  beam direction but there are no major differences.}

\begin{figure}[hbtp]%
  \centering
  \subfloat[complete spectrum]{{\includegraphics[width=0.4 \textwidth]{../images/y_distribution.pdf}}}%
  \qquad
  \subfloat[spectrum at 2 and 8 GeV]{{\includegraphics[width=0.4 \textwidth]{../images/y_e28_distribution.pdf} }}%
  \caption{\added{Distribution of summed up traces perpendicular to  the direction of the incoming particle}}%
  \label{fig:y-distri}%
\end{figure}

\added{Figure \ref{fig:y-distri} shows the distribution of the summed
  traces in the individual cells perpendicular to the beam
  direction. It can be seen that there is no energy information
...
\ref{fcn_structure}.  \added{Different structures are tried out. The
...
nodes have ReLU as their activation function. \added{The linear output
  node is choosen to serve the regression task.}
...
\begin{figure}[hbtp]
  ...
  \caption{ The results of the neural network and the linear fit are
    ...
    bin divided by the root of the mean energy. \added{The graph shows
...
\added{Convergence problems occurred with the resulting loss function
  ...
  optimum. Therefore the network is pre-trained with the MSE until it
...
\added{Altogether it could be shown that a better resolution for
  ...
  demonstrated. Adversarial training is in this case not successful. A
  method could be described how a corresponding loss function could be
...
set are created without pile up. \added{For comparison the $P_T$
  results of the anti kT-Algorithm is used and denoted as $P_{T,
    \text{Reco}}$.}
...
\added{The regression process in this chapter is iterative. First the
  network is trained with MSE as loss function then the trained
  network is used to calculate the standard deviation $\sigma$, as it
  is needed for the loss function described below. With the later
  described loss function the network is trained again. And again
  $\sigma$ is determined. Then a further time $\sigma$ is determined
  and the network is trained. This training scheme is shown in figure
  \ref{fig:jet-trainings-setup}.}
...
\begin{figure}[hbtp]
  ...
  \end{tikzpicture}
  \caption{\added{The schematic training sequence of the three training steps.}}
    \label{fig:jet-trainings-setup}
\end{figure}

\added{The results will be labeled as MSE, 1st and 2nd. Here MSE means
  that the results are obtained after training with mean squared error
  as a loss function. 1st and 2nd denotes the two subsequent training
  results with the loss function described below.}

To deduce the momenta, the 4 vectors of the particles of the jets are
used as an input for the networks. First, the dimensions of the
\((200, 4)\) input space are flattend into a (800, 1) space. In the
training process, the data is processed in batch sizes of 1024
samples.

\added{The possible network architectures are kept general, they will
  be described later for the two analyzed cases, the JetNet and the
  Particle Flow Network. The basic requirement for the network is that
  the input layer consists of 800 inputs and that the output layer is
  a single linear output node.}
...
\begin{figure}[hbtp]
  ...
  \caption{\added{Example of the $\sigma$ fit. The $\sigma$ values are
...
\added{Another factor that comes to bear is that the value spectrum is
  not flat. This is taken into account by the fact that the loss
  function is binned. The resulting loss function is}
...
per bin. \added{The values are sorted by \(P_{T, \text{Gen}}\) and
  divided into 20 bins, each covering a 5.5 GeV wide spectrum. Per bin
  the mean value is determined and the total loss is the mean of the
  mean values per bin.}  As a result, the individual value ranges in
...
\added{where $X$ stands for the choosen calibration algorithm.}
...
\added{The network is trained with the training procedure described
...
\begin{figure}[hbtp]
  ...
  \caption{The resulting response \(R\) of the Reco algorithm and the
    ...
    displayed. \added{The results are taken from the MSE training
      step.}}
...
\added{The aim of this analysis was to study energy measurements of
