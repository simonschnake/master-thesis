...
#+BEGIN_SRC ipython :session :results raw drawer :exports results :eval no-export
  ...
  jetnet = pickle.load(open('./results/jetnet_binned_results.p', 'rb'))
...
#+BEGIN_SRC ipython :session :results raw drawer :exports results :eval no-export
  ...
  y = binned_statistic(genjet_pt, pfnet['second']['pred']/np.sqrt(genjet_pt), statistic='std', bins=25)[0]
  plt.plot(x, y, 'b-')

  #x = binned_statistic(genjet_pt, pfnet['third']['pred'], statistic='mean', bins=25)[0]
  y = binned_statistic(genjet_pt, pfnet['third']['pred']/np.sqrt(genjet_pt), statistic='std', bins=25)[0]
  plt.plot(x[5:20], y[5:20], 'r-')
...
:RESULTS:
# Out[179]:
: [<matplotlib.lines.Line2D at 0x7f0874ac86a0>]
[[file:./obipy-resources/RISXXJ.png]]
:END:
