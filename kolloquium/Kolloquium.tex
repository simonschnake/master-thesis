...
\begin{frame}{Training Setup}
  ...
  \begin{textblock*}{250pt}(15pt, 20pt)
  \begin{figure}[htp]
    \tikzstyle{arrow} = [draw, -latex']
    \begin{tikzpicture}[]
      \node[punkt] (model) {Model};
      \node[below=2.5cm of model] (1loss) {$\mathcal{L}_{J}(\hat Y, Y, \sigma)$};
      \node[below=0.3 cm of 1loss] (1st) {1st};
      \node[left=2.5cm of 1loss] (mseloss) {$(\hat Y - Y)^2$};
      \node[below=0.3 cm of mseloss] (mse) {MSE};
      \node[right=2.5cm of 1loss] (2loss) {$\mathcal{L}_{J}(\hat Y, Y, \sigma)$};
      \node[below=0.3 cm of 2loss] (2nd) {2nd};

      \node[left=0.5 cm of mseloss] (l) {Loss:};
  ...
  \begin{textblock*}{250pt}(5pt, 190pt)
    \begin{align*}
      \mathcal{L}_J=\ \text{binned} \underbrace{\frac{(y_{\text{true}}-y_{\text{pred}})^2}{\sigma^2}}_{\text{\textcolor{blue}{mean squared weighted error}}} +\underbrace{\ln(\frac{\pi \sigma^2}{2} \left(\text{erf}(\frac{y_{\text{pred}}-a}{\sqrt{2}\sigma}) - \text{erf}(\frac{y_{\text{pred}}-b}{\sqrt{2}\sigma})\right)^2)}_{\text{\textcolor{red}{boundary correction term}}}
    \end{align*}
  \end{textblock*}
\end{frame}
