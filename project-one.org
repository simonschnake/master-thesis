...
#+BEGIN_SRC ipython :session :results raw drawer :exports none :eval no-export
  ...
  results = D.predict_generator(DataGenerator(X_test, Y_test,
					      batch_size=128, random_flip=False,
...
#+BEGIN_SRC ipython :session :results raw drawer :exports none :eval no-export
  ...
  y_fit = fitfunc(c_fit, np.sum(X_test[:len(results)], axis=1)).reshape(len(results),)
  n = 20
...
#+BEGIN_SRC ipython :session :results raw drawer :exports none :eval no-export
  ...
  ax.plot(y_f, mu_f - y_f, '-', color='#1f77b4')
  ax.plot(y_nn, mu_nn - y_nn, 'k-')
  plt.text(y_f[-1] + 0.1, mu_f[-1] - y_f[-1]+0.005, 'linear fit', ha='left', va='center', size=15, color='#1f77b4')
  plt.text(y_nn[-1] + 0.1, mu_nn[-1] - y_nn[-1]-0.005, 'neural net', ha='left', va='center', size=15)
  plt.ylabel(r'$\mu - E_{\text{true}}$ [GeV]')
  ax.xaxis.set_ticks([])
  ax.spines["top"].set_visible(False)
  ax.spines["right"].set_visible(False)
