...
[[file:./.ob-jupyter/ec48379ea89bf106bf38a3ac47fb9c564d7a8fbc.png]]
:END:
...
#+BEGIN_SRC jupyter-python :async "yes" :session "py" :results raw drawer :exports none :eval no-export
  entries  = X_test[:10000]
  energies = np.transpose(Y_test[:10000])[0]
  plt.hist(energies, bins=np.arange(0., 10.1, 1), density=True, alpha=0.75, color='red')
  plt.xlabel(r'$E_{\text{true}}$ [GeV]')
  plt.ylabel(r'norm. to 1')
  plt.xlim([-0.1, 10.1])
  plt.ylim([0., 0.5])
  plt.savefig('images/energy_distribution.pdf')
#+END_SRC

#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/7df237a1a933e96740857d0352739fa74134ccac.png]]
:END:
...
#+BEGIN_SRC jupyter-python :async "yes" :session "py" :results raw drawer :exports none :eval no-export
  entries  = X_test[:10000]
  energies = np.transpose(Y_test[:10000])[0]
  sum_n = np.sum(entries, axis=1)
  # entries.reshape((entries.shape[0], 17, 8, 8))
  
  plt.hist(sum_n, bins=np.arange(1, 372, 5), density=True, alpha=0.75, color='red')
  plt.xlabel(r'$\sum_i n_i$')
  plt.ylabel('norm. to 1')
  # plt.xlim([0., 10.])
  # plt.ylim([0., 0.01])
  plt.savefig('images/sumn_distribution.pdf')
  
#+END_SRC

#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/ec5d76545ea959359be01f79741f3cd07afd2621.png]]
:END:
...
#+BEGIN_SRC jupyter-python :async "yes" :session "py" :results raw drawer :exports none :eval no-export
  ...
  sum_n_e_8 = sum_n[(energies > 7.9) * (energies < 8.1)]

  plt.hist(sum_n_e_8, bins=np.arange(1, 372, 5), density=True, alpha=0.5, color='red')

  sum_n_e_2 = sum_n[(energies > 1.9) * (energies < 2.1)]

  plt.hist(sum_n_e_2, bins=np.arange(1, 372, 5), density=True, alpha=0.5, color='blue')

  plt.xlabel(r'$\sum_i n_i$')
  
  plt.ylabel('norm. to 1')
  
  plt.text(75, 0.03, r'$2\pm 0.1$ GeV', color='blue', size=13)
  plt.text(250, 0.015, r'$8\pm 0.1$ GeV', color='red', size=13)
  # plt.xlim([0., 10.])
  # plt.ylim([0., 0.01])
  plt.savefig('images/sumn_e28_distribution.pdf')
#+END_SRC

#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/b48b36805c9c714110b1684c74c9b6b83d8c9cf4.png]]
:END:

#+BEGIN_SRC jupyter-python :async "yes" :session "py" :results raw drawer :exports none :eval no-export
  entries_in_x = entries.reshape((entries.shape[0], 17, 64))
  
  sum_n = np.sum(entries_in_x, axis = 2)

  sum_n = np.sum(sum_n, axis=0)
  plt.hist(np.arange(0., 17), weights=sum_n, bins=np.arange(0, 17, 1), density=True, color='red', alpha=0.75)
  
  plt.xlabel(r'layer in x')
  plt.ylabel('norm. to 1')
  
  # plt.xlim([0., 10.])
  # plt.ylim([0., 0.01])
  plt.savefig('images/x_distribution.pdf')
#+END_SRC

#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/02ad5474b3b6891b64eed9bc89da47a1b180f90b.png]]
:END:
...
#+BEGIN_SRC jupyter-python :async "yes" :session "py" :results raw drawer :exports none :eval no-export
  ...
  sum_n_e_2 = sum_n[(energies > 1.9) * (energies < 2.1)]
  sum_n_e_2 = np.sum(sum_n_e_2, axis=0)
  plt.hist(np.arange(0., 17), weights=sum_n_e_2, bins=np.arange(0, 17, 1), density=True, alpha=0.5, color='blue')

  sum_n_e_8 = sum_n[(energies > 7.9) * (energies < 8.1)]
  sum_n_e_8 = np.sum(sum_n_e_8, axis=0)
  plt.hist(np.arange(0., 17), weights=sum_n_e_8, bins=np.arange(0, 17, 1), density=True, alpha=0.5, color='red')
  
  plt.xlabel(r'layer in x')
  plt.ylabel('norm. to 1')
  
  plt.text(4, 0.3, r'$2\pm 0.1$ GeV', color='blue', size=13)
  plt.text(5, 0.15, r'$8\pm 0.1$ GeV', color='red', size=13)
  # plt.xlim([0., 10.])
  # plt.ylim([0., 0.01])
  plt.savefig('images/x_e28_distribution.pdf')
#+END_SRC


#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/5b5e1a9e742c1556feb04885043c471f191c5f0c.png]]
:END:

#+BEGIN_SRC jupyter-python :async "yes" :session "py" :results raw drawer :exports none :eval no-export
  entries = entries.reshape((entries.shape[0], 17, 8, 8))
  
  sum_n = np.sum(entries, axis = 1)
  sum_n = np.sum(sum_n, axis = 1)

  sum_n_e_2 = sum_n[(energies > 1.9) * (energies < 2.1)]
  sum_n_e_2 = np.sum(sum_n_e_2, axis=0)
  plt.hist(np.arange(0., 8.), weights=sum_n_e_2, bins=np.arange(0.1, 8.1, 1), density=True, alpha=0.5, color='blue')

  sum_n_e_8 = sum_n[(energies > 7.9) * (energies < 8.1)]
  sum_n_e_8 = np.sum(sum_n_e_8, axis=0)
  plt.hist(np.arange(0., 8), weights=sum_n_e_8, bins=np.arange(0.1, 8.1, 1), density=True, alpha=0.5, color='red')
  
  plt.xlabel(r'cell in y')
  plt.ylabel('norm. to 1')
  
  plt.text(4.5, 0.3, r'$2\pm 0.1$ GeV', color='blue', size=13)
  plt.text(4.5, 0.2, r'$8\pm 0.1$ GeV', color='red', size=13)
  # plt.xlim([0., 10.])
  # plt.ylim([0., 0.01])
  plt.savefig('images/y_e28_distribution.pdf')
#+END_SRC

#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/0472836cf18e2fd4b48a2e035abf461b5a681a95.png]]
:END:
...
#+BEGIN_SRC jupyter-python :async "yes" :session "py" :results raw drawer :exports none :eval no-export
  ...
  plt.hist(np.arange(0., 8.), weights=sum_n, bins=np.arange(0.1, 8.1, 1), density=True, color='red', alpha=0.75)
  ...
  # plt.xlim([0., 10.])
  # plt.ylim([0., 0.01])
  plt.savefig('images/y_distribution.pdf')
#+END_SRC

#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/cb3bc8753f620dc814c526963e7eb9689eef254e.png]]
:END:
...
#+BEGIN_SRC jupyter-python :async "yes" :session "py" :results raw drawer :exports none :eval no-export
  ...
  plt.savefig('images/data_display.pdf')
#+END_SRC

#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/28eb50bfe56445d84e2dcfd4f7d322119a3da094.png]]
:END:
...
#+BEGIN_SRC jupyter-python :async "yes" :session "py" :results raw drawer :exports none :eval no-export
  ...
  sum_n = np.sum(entries.reshape(entries.shape[0], 1088), axis=1)
  ...
  plt.plot(sum_n, energies, 'r.', alpha=0.06)
  plt.plot(n, inv_fitfunc(c_fit1, n), 'k-', linestyle='dashed')     # Fit
...
#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/0c20c83864c0ff98204ee83cdde19ebd65309d89.png]]
:END:
...
:RESULTS:
: Text(0.5, 0, '$\\sum_i n_i$')
[[file:./.ob-jupyter/d2b54c919e40184118b3d7c9ce8126abc9838e55.png]]
:END:
...
#+BEGIN_SRC jupyter-python :async "yes" :session "py" :results raw drawer :exports none :eval no-export
  ...
  def likelihood(par):
      ...
      sigma = 0.4*np.sqrt(np.abs(mu)) # 2/par[0] 
  ...
  def mwse(par):
      ...
      return -np.sum(np.log(norm.pdf(energies, mu, sigma)+epsilon))

  def mse(par):
      epsilon = 0.0000001
      mu = sum_n*par[0]+par[1]
      sigma = 1/4. #np.sqrt(np.abs(mu)) # 2/par[0] 
      return -np.sum(np.log(norm.pdf(energies, mu, sigma)+epsilon))
  ...
  out = optimize.minimize(mse, np.array(c_fit), method='BFGS', tol=1)
  c_mse = out['x']

  out = optimize.minimize(mwse, np.array(c_fit), method='BFGS', tol=1)
  c_mwse = out['x']

  out = optimize.minimize(likelihood, np.array(c_mse), method='BFGS', tol=1e-3)
  c_like = out['x']


  n1 = np.arange(0, 255, 0.5)
  n2 = np.arange(0, 240, 0.5)
  n3 = np.arange(0, 245, 0.5)
  plt.plot(sum_n, energies, 'r.', alpha=0.06)
  plt.plot(n1, fitfunc(c_mse, n1), 'k-')     # Fit
  plt.plot(n2, fitfunc(c_like, n2), 'b-')     # Fit
  ...
  plt.text(140, 10, 'likelihood fit', ha='left',size=16, color='b')
  plt.text(254, 10, 'mse', ha='left',size=16)

  plt.savefig('images/e-vs-sum_n_like.pdf')
#+END_SRC

#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/1b219d7960a34a029f2640608c0edc36b8c53a21.png]]
:END:
...
#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/3d02ecc4c5e0654752aefd1d480e13354344c444.png]]
:END:
...
#+BEGIN_SRC jupyter-python :async "yes" :session "py" :results raw drawer :exports none :eval no-export
  ...
  plt.ylabel(r'$\sigma / \sqrt{E_{\text{true}}}$ [GeV]')
  plt.xlabel(r'$E_{\text{true}}$ [GeV]')
  ax.spines["top"].set_visible(False)
  ax.spines["right"].set_visible(False)
...
<ipython-input-11-42a753b75ced> in <module>
      1 import sys
      2 sys.path.append('./src')
----> 3 from utils import *
      4 y_true = np.array(Y_test)[:len(results)].reshape(len(results),)
      5 y_pred = results.reshape(len(results),)

~/master-thesis/src/utils.py in <module>
      1 # this file is for outsourcing python functions, to keep our notebooks clean
----> 2 import keras
