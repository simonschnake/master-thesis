...
#+BEGIN_SRC ipython :session :results raw drawer :exports none :eval no-export
  ...
  plt.plot(epochs_D, history['val_D_loss'], '-', color='#1f77b4')
  plt.text(float(epochs_D[-1])+0.5, history['D_loss'][-1], 'training loss', ha='left', va='center', size=15)
  plt.text(float(epochs_D[-1])+0.5, history['val_D_loss'][-1], 'validation loss', ha='left', va='center', size=15, color='#1f77b4')
  plt.xlabel('epochs')
  plt.ylabel('loss')
#+END_SRC
