...
:RESULTS:
# Out[24]:
[[file:./obipy-resources/Gut6Hg.png]]
:END:
...
#+BEGIN_SRC ipython :session :results raw drawer :exports none :eval no-export
  ...
  #plt.plot(Y_train[:10000], (D.predict(X_train[:10000])-Y_train[:10000]), '.', alpha=0.25, markersize=3, color='#1f77b4')
...
#+BEGIN_SRC ipython :session :results raw drawer :exports none :eval no-export
  ...
  y_fit = fitfunc(c_fit, np.sum(X_test, axis=1)).reshape(len(y_true),)
  n = 20
...
#+BEGIN_SRC ipython :session :results raw drawer :exports none :eval no-export
  ...
  fig, ax = plt.subplots()
  plt.plot(energies, fitfunc(c_like, sum_n) - energies, '.', alpha=0.25, markersize=3, color='#1f77b4')
  # plt.plot(Y_train[:10000], (D.predict(X_train[:10000])-Y_train[:10000]), 'r.', alpha=0.25, markersize=3)
  plt.plot(Y_test, (D.predict(X_test)-Y_test), 'k.', alpha=0.25, markersize=3)
  # plt.ylim([-6., 6])
  # plt.xlim([0.,10])
  plt.ylabel(r'$E_{\text{pred}} - E_{\text{true}}$ [GeV]')
  plt.xlabel(r'$E_{\text{true}}$ [GeV]')
...
#+BEGIN_SRC ipython :session :results raw drawer :exports none :eval no-export
  ...
  n = 20
  y_f, mu_f, sigma_f = sliced_statistics(y_true, y_fit, n) 
  y_lh, mu_lh, sigma_lh = sliced_statistics(y_true , y_pred, n) 

  fig = plt.figure()
  ax = fig.add_subplot(2,1,1)

  ax.plot(y_f, mu_f - y_f, '-', color='#1f77b4')
  ax.plot(y_lh, mu_lh - y_lh, 'k-')
  plt.text(y_f[-1] + 0.1, mu_f[-1] - y_f[-1], 'linear fit', ha='left', va='center', size=15, color='#1f77b4')
  plt.text(y_lh[-1] + 0.1, mu_lh[-1] - y_lh[-1], 'likelihood loss', ha='left', va='center', size=15)
  plt.ylabel(r'$\mu - E_{\text{true}}$ [GeV]')
  ax.xaxis.set_ticks([])
  ax.spines["top"].set_visible(False)
  ax.spines["right"].set_visible(False)
  ...
  ax.spines["right"].set_visible(False)
  plt.text(y_f[-1] + 0.1, sigma_f[-1] / np.sqrt(y_f[-1]), 'linear fit', ha='left', va='center', size=15, color='#1f77b4')
  plt.text(y_lh[-1] + 0.1, sigma_lh[-1] / np.sqrt(y_lh[-1]), 'likelihood loss', ha='left', va='center', size=15)
  plt.ylim([0., 1.5])
