...
#+BEGIN_SRC ipython :session :results raw drawer :exports none :eval no-export
  ...
  epochs_D = range(len(history['D_loss']))
  epochs_D_val = range(len(history['val_D_loss']))
  fig, ax = plt.subplots()
  ax.spines['top'].set_visible(False)
  ax.spines['bottom'].set_visible(False)
  ax.spines['right'].set_visible(False)
  ...
  plt.tick_params(axis='both', which='both', bottom=False, top=False,
                  labelbottom=True, left=True, right=False, labelleft=True)
  ax.xaxis.set_major_formatter(plt.FuncFormatter('{:.0f}'.format))
  plt.plot(epochs_D, history['D_loss'], 'k-')
  plt.plot(epochs_D_val, history['val_D_loss'], '-', color='#1f77b4')
  plt.text(float(epochs_D[-1])+0.5, history['D_loss'][-1], 'training loss', ha='left', va='center', size=15)
  plt.text(float(epochs_D_val[-1])+0.5, history['val_D_loss'][-1], 'validation loss', ha='left', va='center', size=15, color='#1f77b4')
  plt.xlabel('epochs')
  plt.ylabel('loss')
#+END_SRC
